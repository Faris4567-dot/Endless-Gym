...
const DashboardPage = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (
        memberStatsRes.data.success &&
        inquiryStatsRes.data.success &&
        trainerStatsRes.data.success &&
        programStatsRes.data.success
      ) {
        setStats({
          members: {
            total: memberStatsRes.data.stats.total || 0,
            active: memberStatsRes.data.stats.active || 0,
            expired: memberStatsRes.data.stats.expired || 0,
            pending: memberStatsRes.data.stats.pending || 0,
            monthlyData:
              memberStatsRes.data.stats.monthlyData || [],
          },

          inquiries: {
            total: inquiryStatsRes.data.stats.total || 0,
            new: inquiryStatsRes.data.stats.new || 0,
            contacted:
              inquiryStatsRes.data.stats.contacted || 0,
            converted:
              inquiryStatsRes.data.stats.converted || 0,
          },

          trainers: {
            total: trainerStatsRes.data.stats.total || 0,
          },

          programs: {
            total: programStatsRes.data.stats.total || 0,
          },
        });
      }
      ...
      if (inquiriesRes.data.success) {
        setRecentInquiries(
          (inquiriesRes.data.inquiries || []).slice(0, 5)
        );
      }
    } catch (error) {
  ...
  const monthlyData = stats.members.monthlyData.map((item) => ({
    name: monthNames[item._id - 1] || "Unknown",
    members: item.count || 0,
  }));

  // ---------------------------------------
  // REAL MEMBERSHIP STATUS DATA
  // ---------------------------------------
